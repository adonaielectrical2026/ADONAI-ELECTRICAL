(function () {
  ...
  function comprobarEnsayoCircuito(c, draft) {
    ...
    if (polaridadAplica) {
      ...
      else if (c.polaridad==='no_aplica') {
        if (!String(c.polaridadMotivo||'').trim()) pendientes.push('Polaridad: declarada "no aplica", falta que el técnico aclare el motivo.');
        else notas.push('Polaridad: no aplica — ' + c.polaridadMotivo + '.');
      }
    ...
    if (secuenciaAplica) {
      ...
      else if (c.secuenciaFases==='no_aplica') {
        if (!String(c.secuenciaFasesMotivo||'').trim()) pendientes.push('Secuencia de fases: declarada "no aplica", falta que el técnico aclare el motivo.');
        else notas.push('Secuencia de fases: no aplica — ' + c.secuenciaFasesMotivo + '.');
      }
  ...
  function renderCircuitosList() {
    ...
    draft.circuitos.forEach((c) => {
      ...
      card.innerHTML =
        ...
        '<div class="field"><label>Ensayo de polaridad</label><select class="select" data-f="polaridad"><option value="pendiente"'+((c.polaridad||'pendiente')==='pendiente'?' selected':'')+'>Pendiente</option><option value="si"'+(c.polaridad==='si'?' selected':'')+'>Correcta</option><option value="no"'+(c.polaridad==='no'?' selected':'')+'>Incorrecta</option><option value="no_aplica"'+(c.polaridad==='no_aplica'?' selected':'')+'>No aplica — aclarar motivo</option></select></div>' +
        (c.polaridad==='no_aplica' ? '<div class="field"><label>Motivo (polaridad no aplica)</label><input class="input" data-f="polaridadMotivo" placeholder="Ej: obra residencial, tomas/luces con dispositivos bipolares" value="' + escapeHtml(c.polaridadMotivo || '') + '"></div>' : '') +
        '<div class="field"><label>Secuencia de fases</label><select class="select" data-f="secuenciaFases"><option value="pendiente"'+((c.secuenciaFases||'pendiente')==='pendiente'?' selected':'')+'>Pendiente / no aplica</option><option value="si"'+(c.secuenciaFases==='si'?' selected':'')+'>Correcta</option><option value="no"'+(c.secuenciaFases==='no'?' selected':'')+'>Incorrecta</option><option value="no_aplica"'+(c.secuenciaFases==='no_aplica'?' selected':'')+'>No aplica — aclarar motivo</option></select></div>' +
        (c.secuenciaFases==='no_aplica' ? '<div class="field"><label>Motivo (secuencia de fases no aplica)</label><input class="input" data-f="secuenciaFasesMotivo" placeholder="Ej: cargas residenciales sin sensibilidad al orden de fases" value="' + escapeHtml(c.secuenciaFasesMotivo || '') + '"></div>' : '') +
      ...
      card.querySelectorAll('[data-f]').forEach((input) => {
        input.addEventListener('change', () => {
          ...
          const esTexto = f === 'nombre' || f === 'material' || f === 'metodo' || f === 'uso' || f === 'aislacion' ||
            f === 'disposicion' || f === 'montaje' || f === 'tipoProteccion' || f === 'equipo' || f === 'modoCargaVe' || f === 'diferencialIndividualVe' || f === 'faseAsignada' || f === 'curvaProteccion' || f === 'selectividadFabricante' || f === 'continuidadPe' || f === 'polaridad' || f === 'polaridadMotivo' || f === 'secuenciaFases' || f === 'secuenciaFasesMotivo' || f === 'ensayoRcd';
          if (f === 'fasesCircuito') {
    ...
    if (interno && trabajo) {
      ...
      if (pgPdf.aplica) {
        ...
        const filasProto=(trabajo.circuitos||[]).map((c,i)=>{const r=comprobarEnsayoCircuito(c,trabajo);return [
          ...
          r.polaridadAplica?(c.polaridad==='si'?'OK':(c.polaridad==='no'?'No':(c.polaridad==='no_aplica'?'N/A*':'Pend.'))):'N/A',
          r.secuenciaAplica?(c.secuenciaFases==='si'?'OK':(c.secuenciaFases==='no'?'No':(c.secuenciaFases==='no_aplica'?'N/A*':'Pend.'))):'N/A',
