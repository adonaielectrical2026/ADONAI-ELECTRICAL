(function () {
  ...
  const REFERENCIAS_NORMATIVAS = [
    ...
    { rango: 'principal', tema: 'Protección contra cortocircuitos',
      cita: 'UTE, RBT Capítulo V numeral 1.b: en el origen de todo circuito debe haber una protección con capacidad de corte acorde a la corriente de cortocircuito prevista. El Anexo del Capítulo II §7 permite verificar térmicamente el conductor frente al cortocircuito.' },
  ...
  function comprobarCircuito(p, opciones) {
    ...
    r.iccKa = Number(p.iccKa) > 0 ? Number(p.iccKa) : null;
    r.poderCorteKa = Number(p.poderCorteKa) > 0 ? Number(p.poderCorteKa) : null;
    ...
    r.cumplePoderCorte = (r.iccKa !== null && r.poderCorteKa !== null) ? r.poderCorteKa >= r.iccKa : null;
    ...
    if (material === 'cobre') {
      ...
      else if (r.iccKa !== null && r.tiempoDespejeS !== null) r.i2tExigido = Math.pow(r.iccKa * 1000, 2) * r.tiempoDespejeS;
    }
  ...
  function calcularProteccionGeneral(draft) {
    ...
    return { ...comun, termicaIn, diferencialIn, coordina: true };
  }
  ...
  function tieneAvanzados(c) {
    return Boolean(c.expuestoSol || c.inProteccion || (c.tipoProteccion && c.tipoProteccion !== 'mcb') ||
      c.iccKa || c.poderCorteKa || c.i2tPasante || c.tiempoDespejeS);
  }
  ...
  function renderCircuitosList() {
    ...
    draft.circuitos.forEach((c) => {
      const calc = calcularCircuito(c, caidaPrevia);
      const comp = calc.apto ? comprobarCircuito(datosCircuito(c, caidaPrevia)) : null;
      ...
      card.innerHTML =
        ...
        ((c.tipoProteccion || 'mcb') === 'otro'
          ...
          : '') +
        '<div class="field"><label>Icc prevista (kA)</label><input class="input" type="number" step="0.1" min="0" data-f="iccKa" value="' + (Number(c.iccKa) || '') + '"></div>' +
        '<div class="field"><label>Poder de corte (kA)</label><input class="input" type="number" step="0.5" min="0" data-f="poderCorteKa" value="' + (Number(c.poderCorteKa) || '') + '"></div>' +
        ...
        '<div style="margin-top:16px">' + (calc.apto
          ? '<div class="panel-dark card-pad" style="display:flex;flex-wrap:wrap;gap:20px">' +
            ...
            statBox('Protección', calc.breaker + ' A', true) + statBox('Curva sugerida', calc.curva) +
  ...
  function renderComprobacion(c) {
    ...
    let html = '<div class="light-stat-grid">' +
      ...
      filaComprobacion('Poder de corte', c.cumplePoderCorte,
        (c.poderCorteKa === null ? '—' : fmt(c.poderCorteKa) + ' kA') + ' >= Icc ' + (c.iccKa === null ? '—' : fmt(c.iccKa) + ' kA'),
        'UTE RBT Cap. V §1.b') +
  ...
  async function construirPdfPresupuesto(p, opciones) {
    ...
    if (interno && trabajo && trabajo.circuitos && trabajo.circuitos.length) {
      ...
      doc.autoTable({
        startY: y,
        margin: margenTablas,
        head: [['Circuito', 'Sección', 'Ib', 'In', 'Iz corr.', 'Caída total', 'Estado']],
      ...
      const refs = doc.splitTextToSize(pdfTexto(
        ...
        'Cortocircuito (Capítulo V §1.b y Anexo §7): se verifica sólo cuando se cargan la corriente de cortocircuito prevista, el poder de corte y la energía pasante del termomagnético.'),
        pageWidth - 2 * margin);
