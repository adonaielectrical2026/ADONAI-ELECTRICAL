(function () {
  ...
  // resuelve la app. K es la conductividad a 25 °C que da el propio reglamento.
  const CONDUCTIVIDAD_UTE = { cobre: 56.9, aluminio: 34.7 };
  ...
  function comprobarCircuito(p, opciones) {
    ...
    const inProt = Number(o.in) || base.breaker || null;
    const fases = p.fases || 1;
    const material = p.material === 'aluminio' ? 'aluminio' : 'cobre';
    const aislacion = p.aislacion === 'xlpe' ? 'xlpe' : 'pvc';
    const categoria = CATEGORIA_METODO[p.metodo] || 'conducto';
  ...
  function generarMateriales(circuitos, draft) {
    ...
    circuitos.forEach((c) => {
      ...
      // del tramo y no el largo por conductor.
      const multi = CABLE_POR_METODO[c.metodo];
      const seccionTxt = fmt(calc.seccionAdoptada, calc.seccionAdoptada < 10 ? 1 : 0).replace(/,00$/, '');
      if (multi) {
        const medida = conductores + 'x' + calc.seccionAdoptada;
        const largoMulti = Math.ceil((Number(c.l) || 0) * 1.1);
        if (largoMulti > 0) {
          add('Cable ' + multi.etiqueta + ' ' + conductores + 'x' + seccionTxt + ' mm²', 'm', largoMulti,
              precios[multi.precio][medida] || 0);
        }
      } else {
        const largoCable = Math.ceil((Number(c.l) || 0) * conductores * 1.1);
        if (largoCable > 0) add('Cable unipolar ' + seccionTxt + ' mm²', 'm', largoCable, precios.cableUnipolar[calc.seccionAdoptada] || 0);
      }
  ...
  function renderComprobacion(c) {
    ...
    const izTxt = fmt(c.izTabla) + ' A x ' + fmt(c.ft, 2) + ' (temp.) x ' + fmt(c.fa, 2) + ' (agrup.) = ' + fmt(c.iz) + ' A';
    ...
    let html = '<div class="light-stat-grid">' +
      ...
      filaComprobacion('Sobrecarga (I2)', c.cumpleI2 !== false,
        'I2 ' + (c.i2 === null ? '—' : fmt(c.i2) + ' A') + ' <= 1,45 · Iz ' + fmt(c.limite145) + ' A',
        'I2 = 1,45 · In para termomagnéticos IEC 60898') +
      filaComprobacion('Caída de tensión', c.cumpleCaida,
        'ΔU ' + fmt(c.dU) + ' V = ' + fmt(c.dUPct) + ' % <= ' + fmt(c.caidaMax) + ' %',
        'UTE RBT Cap. II - Anexo §8') +
      '</div>';
    html += '<div style="margin-top:12px">' + (c.verificado
  ...
  async function construirPdfPresupuesto(p, opciones) {
    ...
    if (interno && trabajo && trabajo.circuitos && trabajo.circuitos.length) {
      const filasComp = trabajo.circuitos.map((c) => {
        const comp = comprobarCircuito({
          ib: c.ib, v: c.v, fases: c.fases, l: c.l, material: c.material, metodo: c.metodo,
          aislacion: c.aislacion, tempAmb: c.tempAmb, agrupados: c.agrupados, cosPhi: c.cosPhi,
        ...
        return [c.nombre || 'Circuito',
                ...
                fmt(comp.iz) + ' A',
                fmt(comp.dUPct) + ' %',
                comp.verificado ? 'Verificado' : 'Revisar'];
      });
      lugarPara(4 + FILA * 3);
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(111, 114, 119);
      ...
      doc.autoTable({
        startY: y,
        margin: margenTablas,
        head: [['Circuito', 'Sección', 'Ib', 'In', 'Iz corr.', 'ΔU', 'Estado']],
      ...
      const refs = doc.splitTextToSize(pdfTexto(
        'Cómo se comprobó cada circuito. Capacidad de conducción: Iz = I de tabla x factor de temperatura x factor de agrupamiento, y se verifica Iz >= Ib ' +
        '(Reglamento de Baja Tensión de UTE, Capítulo II - Anexo, Tablas VI a XIII, Tabla XIV de temperatura y §5.1 de agrupamiento). ' +
        'Coordinación con la protección: Ib <= In <= Iz (UTE, Capítulo V numeral 1.a, que exige que la protección garantice el límite admisible del conductor; ' +
        'formulación explícita en IEC 60364-4-43:2023 §431.4.2, como referencia técnica complementaria, que agrega I2 <= 1,45 Iz, cumplida porque en los ' +
        'termomagnéticos IEC 60898-1 es I2 = 1,45 In). Caída de tensión: e = 2LW/(KSV) en monofásico y LW/(KSV) en trifásico, con K a 25 °C de 56,9 (Cu) y ' +
        '34,7 (Al), contra un máximo de 3 % en alumbrado y 5 % en los demás usos (UTE, Capítulo II - Anexo, numeral 8).'),
        pageWidth - 2 * margin);
